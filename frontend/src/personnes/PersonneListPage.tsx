...
interface PersonneListPageProps {
  tenantId: string
  onSelect: (personne: Personne) => void
  onSelectDossier?: (id: string) => void
}

export function PersonneListPage({ tenantId, onSelect, onSelectDossier }: PersonneListPageProps) {
  ...
  async function handleSave(values: PersonneFormValues) {
    ...
    const { error } = await supabase.from('personnes').insert(payload)
    setSaving(false)
