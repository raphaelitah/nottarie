...
interface ImmeubleListPageProps {
  tenantId: string
  onSelect: (immeuble: Immeuble) => void
  onSelectDossier?: (id: string) => void
}

export function ImmeubleListPage({ tenantId, onSelect, onSelectDossier }: ImmeubleListPageProps) {
  ...
  async function handleSave(values: ImmeubleFormValues) {
    ...
    const { error } = await supabase.from('immeubles').insert(payload)
    setSaving(false)
